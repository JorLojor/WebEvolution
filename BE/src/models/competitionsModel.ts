...
export const inputDataCompetitions = async (RegistrationID: number, newDataCompetitions: Competition): Promise<number> => {
  const { Pernyataan_Originalitas, Proposal, Dokumen_Substansi, title } = newDataCompetitions;
  ...
  if (dataCompetitions.length > 0) {
    ...
    try {
      ...
      if (Pernyataan_Originalitas !== '') {
        ...
        console.log('Update Pernyataan_Originalitas:', Pernyataan_Originalitas);
        hasil = 200;
      }
...
export const checkCompetitionsByRegistrationID = async (RegistrationID: number): Promise<number> => {
  try {
    ...
    if (Pernyataan_Originalitas !== '' && Proposal !== '' && Dokumen_Substansi !== '') {
...
export const createCompetitions = async (RegistrationID: number, title: string): Promise<number> => {
  try {
