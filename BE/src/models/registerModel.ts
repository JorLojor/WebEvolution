...
export const checkStatusRegistrasiWithExpectedStatus = async (
     ...
     const [dataRegister]: any = await DBconnection.query(
          "SELECT Status_Registrasi FROM Register WHERE RegistrationID = ?",
          [RegistrationID]
     );
...
export const changeStatusRegistrasi = async (
     RegistrationID: number,
     newStatus: number
): Promise<void> => {
     await DBconnection.query(
          "UPDATE Register SET Status_Registrasi = ? WHERE RegistrationID = ?",
     ...
     try {
          ...
          if (dataRegister.length > 0) {
               const token = jsonwebtoken.sign(
                    { RegistrationID: dataRegister[0].RegistrationID },
               ...
               await DBconnection.query(
                    "UPDATE Register SET token = ? WHERE RegistrationID = ?",
                    [token, dataRegister[0].RegistrationID]
               );
...
export const checkStatusRegistrasi = async (
     RegistrationID: number
): Promise<string> => {
     const [dataRegister]: any = await DBconnection.query(
          "SELECT Status_Registrasi FROM Register WHERE RegistrationID = ?",
...
export const getSingleRegister = async (
     RegistrationID: number
): Promise<Register | null> => {
     const [dataRegister]: any = await DBconnection.query(
          "SELECT * FROM Register WHERE RegistrationID = ?",
          [RegistrationID]
     );
     if (dataRegister.length > 0) {
          return dataRegister[0] as Register;
...
export const updateRegister = async (
     RegistrationID: number,
     updatedRegister: Partial<Register>
): Promise<void> => {
     const {
          Nama,
          Nomor_Telfon,
