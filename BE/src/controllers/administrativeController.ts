...
export const uploadAdministrativeController = async (req: Request, res: Response) => {
    try {
        ...
        const cek_status = await checkStatusRegistrasiWithExpectedStatus(RegistrationID,1);
        if (cek_status !== 1) {
        ...
        await inputDataAdministrative(RegistrationID, newDataAdministrative);



        return res.status(200).json({ message: 'Documents uploaded successfully.', data: newDataAdministrative });

    } catch (error) {
        console.error(error, "\n   backend error broo bagian administative controller");
        res.status(500).json({ message: "backend error broo bagian team administrative" });
    }
};
